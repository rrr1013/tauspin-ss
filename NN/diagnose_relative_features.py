...
LOGGER = logging.getLogger("diagnose_relative_features")
FEATURE_SETS = ("absolute-v1", "absolute-plus-parent-relative-v1")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare the Current Transformer with and without parent-tau "
            "relative track/PFO features."
        )
    )
    ...
    parser.add_argument(
        ...
        default=(
            OUTPUT_DIR
            / "diagnostics"
            / "fixed-partial-v1-ptmatched20-relative-v1-5000-step-v1"
        ),
    )
...
def validate_inputs(
    ...
    relative = metadata["absolute-plus-parent-relative-v1"]
    for key in (
        "counts",
        "labels",
        "split_fractions",
    ...
    if relative.get("feature_set") != "absolute-plus-parent-relative-v1":
    ...
    for group in ("event", "tau"):
        if absolute["feature_names"][group] != relative["feature_names"][group]:
            raise ValueError(f"Unexpected {group} feature difference")
    ...
    return {
        ...
        "relative_feature_schema_verified": True,
...
def checkpoint_document(
    *,
    ...
    return {
        "format_version": 1,
        "purpose": (
            "Parent-tau relative-feature ablation on the fixed partial "
            "20 GeV pT-matched sample; not a final physics result."
        ),
...
def save_plot(
    ...
    colors = {
        ...
        "absolute-plus-parent-relative-v1": "tab:orange",
    }
    labels = {
        "absolute-v1": "Absolute baseline",
        "absolute-plus-parent-relative-v1": "+ parent-tau relative",
    }
    ...
    for axis in axes:
        ...
        axis.legend()
    figure.suptitle(
        "Parent-tau relative-feature ablation "
...
def main() -> None:
    ...
    processed_dirs = {
        ...
        "absolute-plus-parent-relative-v1": (
    ...
    config = {
        "purpose": (
            "Test whether explicit parent-tau relative track/PFO features "
            "improve the Current Transformer without changing its structure."
        ),
    ...
    series = {
        feature_set: summarize(results[feature_set]["validation_history"])
        for feature_set in FEATURE_SETS
    }
    baseline = series["absolute-v1"]
    relative = series["absolute-plus-parent-relative-v1"]
    summary = {
        "series": series,
