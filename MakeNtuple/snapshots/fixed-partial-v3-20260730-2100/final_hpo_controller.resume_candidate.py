...
DEFAULT_STUDY_NAME = "fixed-partial-v3-hpo-20260730"
DEFAULT_OUTPUT_ROOT = Path("outputs/fixed_partial_v3_hpo")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Continue a terminal study in the same Optuna storage. The "
            "existing data binding and runtime settings must match."
        ),
    )
    parser.add_argument(
        "--status-file", default="controller_status.json"
    )
    parser.add_argument(
        "--completion-file", default="controller_complete.json"
    )
...
def main() -> None:
    ...
    status_path = study_dir / args.status_file
    completion_path = study_dir / args.completion_file
    for name, path in (
        ("--status-file", status_path),
        ("--completion-file", completion_path),
    ):
        if path.parent != study_dir:
            raise ValueError(f"{name} must be a filename inside the study")
    if (
        not args.resume_existing
        and study_dir.exists()
        and any(study_dir.iterdir())
    ):
        raise FileExistsError(
            "Study directory must be absent or completely empty; resuming "
            f"an old study is forbidden: {study_dir}"
        )
    ...
    missing = [str(path) for path in required_bindings if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"Missing binding inputs: {missing}")
    binding = {
        "processed_metadata_path": str(metadata_path.resolve()),
        "processed_metadata_sha256": sha256_file(metadata_path),
    ...
    if args.resume_existing:
        ...
        if not config_path.is_file():
            raise FileNotFoundError(
                f"Existing study config is missing: {config_path}"
            )
        ...
        if mismatches:
            raise ValueError(
                "Cannot resume with changed bindings/settings: "
                f"{mismatches}"
            )
        ...
        if nonterminal:
            raise RuntimeError(
                "Cannot resume while trials are nonterminal: "
                f"{nonterminal}"
            )
        if completion_path.exists():
            raise FileExistsError(
                f"Completion marker already exists: {completion_path}"
            )
        write_json(
            study_dir / "controller_extension_config.json",
            {
                "purpose": (
                    "Deadline-bounded extension of the same validation-only "
                    "HPO study after its original target completed."
                ),
                "started_at": started_at.isoformat(),
    ...
    if not args.resume_existing and not study.trials:
    ...
    if not args.resume_existing:
        write_json(
            ...
            {
            "purpose": (
                "High-throughput validation-only HPO on an explicitly "
                "hash-bound processed dataset and source snapshot."
            ),
