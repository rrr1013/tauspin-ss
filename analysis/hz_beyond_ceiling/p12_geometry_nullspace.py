"""Null-space moments with all reconstructed tau geometry as likelihoods.

Extension of p5 (same null space, same IP response construction): the IP
likelihood is applied to every core track (up to three, response fitted per
track rank on TRAIN), and the PV->SV direction enters through a train-fitted
response density per unit solid angle of the angle between the hypothesis tau
direction and the measured SV direction.  IP and SV factors are multiplied as
if independent.  Measures: flat, ip1 (leading track, = p5 'ip'), ip3, sv,
ip3_sv, ip3_sv_mass, ip3_shuffle.

p5 docstring: Null-space moments with a PV-referenced impact-parameter likelihood.
...
def ip_measurements(tracks: dict, ids: np.ndarray, j: int = 0) -> dict:
    ...
    return {'tdir': tdir, 'e1': e1, 'e2': e2,
            ...
            'n_core': n_core[ids], 'available': np.isfinite(d0) & (n_core[ids] > j)}
...
def fit_sv_response(truth_dir, sv_dir, avail):
    """log density per unit solid angle of angle(truth tau, SV), TRAIN only."""
...
def main() -> None:
    ...
    surf_ids = surface['global_indices'][:n_rows].astype(np.int64)
    responses, metas = [], []
    for j in range(3):
        tm = ip_measurements(tracks, train_ids, j)
        responses.append(fit_response(tm, train_truth, 'psi', 'mag_um'))
        metas.append(ip_measurements(tracks, surf_ids, j))
    ...
    for m in metas:
        flat = m['psi'].reshape(-1)
        m['psi_shuffle'] = flat[perm].reshape(m['psi'].shape)
    with np.load(AUDIT) as g:
        sv_av_all = np.asarray(g['sv_available']) > 0
        sv_dir_all = np.asarray(g['sv_direction'])
        sv_len_all = np.asarray(g['sv_length'])
    sv_ok_all = sv_av_all & (sv_len_all > 1e-6) & np.isfinite(sv_dir_all).all(-1)
    sv_response = fit_sv_response(train_truth, sv_dir_all[train_ids], sv_ok_all[train_ids])
    sv_dir, sv_ok = sv_dir_all[surf_ids], sv_ok_all[surf_ids]
    ...
    llr = {f'ip_track{j}': float(np.mean(ip_log_weight(truth_dir_rows, metas[j], all_rows, responses[j], 'psi', 'mag_um'))) for j in range(3)}
    llr['sv_raw_logdensity_mean_available'] = float(np.mean(sv_log_weight(truth_dir_rows, sv_dir, sv_ok, sv_response)[sv_ok.any(-1)]))
    print(json.dumps({'truth_mean_log_likelihood': llr, 'sv_side_fraction': float(sv_ok.mean())}), flush=True)
    ...
    for start in range(0, n_rows, args.row_chunk):
        ...
        log_ip = [ip_log_weight(tau_dir, metas[j], block, responses[j], 'psi', 'mag_um') for j in range(3)]
        log_sh = sum(ip_log_weight(tau_dir, metas[j], block, responses[j], 'psi_shuffle', 'mag_um') for j in range(3))
        log_ip3 = sum(log_ip)
        ...
        banks = {'flat': ok.astype(np.float64).reshape(-1, rows),
                 'ip1': bank(log_ip[0]), 'ip3': bank(log_ip3), 'sv': bank(log_sv),
                 'ip3_sv': bank(log_ip3 + log_sv), 'ip3_sv_mass': bank(log_ip3 + log_sv + log_m),
                 'ip3_shuffle': bank(log_sh)}
