"""Point-h regression with local-frame geometry channels.
...
def main():
    ...
    (args.output_dir / "config.json").write_text(json.dumps({
        "arm": args.arm, "epochs": args.epochs, "seed": c.SEED, "features": str(args.features),
    ...
    for epoch in range(args.epochs):
        ...
        for cpu_batch in loader:
            ...
            _, h_pred = network(batch)
            loss = nn.functional.mse_loss(h_pred, batch["h"])
        ...
        if args.pilot:
            ...
            return
        metrics, _ = evaluate(network, vloader, device)
        row = {"epoch": epoch + 1, "train_loss": tot_l / tot_n, **metrics}
        history.append(row)
        if len(history) >= 3:
            value = float(np.mean([r["validation_h_mse"] for r in history[-3:]]))
    ...
    metrics, predictions = evaluate(network, vloader, device)
    ...
    tloader = DataLoader(train, batch_size=c.BATCH_SIZE, num_workers=args.workers,
                         collate_fn=collate, pin_memory=True)
    _, tpred = evaluate(network, tloader, device)
    np.savez_compressed(args.output_dir / "train_predictions.npz", **tpred)
