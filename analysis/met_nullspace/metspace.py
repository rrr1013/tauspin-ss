...
# Sensitivity only.  The acceptance-corrected pi x pi moments measured in the
# cross-run synthesis, used to check that the ceiling does not hinge on taking
# the theory values for the spin correlation matrix.
C_HIGGS_MEASURED = np.diag([1.06, 1.12, -0.79])
C_Z_MEASURED = np.diag([0.05, -0.04, 1.08])
...
def moment_estimators(m_minus: np.ndarray, m_plus: np.ndarray,
    ...
    measured_h = 1.0 + np.einsum('ij,rij->r', C_HIGGS_MEASURED, second)
    measured_z = 1.0 + np.einsum('ij,rij->r', C_Z_MEASURED, second)
    return {
        'joint': joint,
        'marginal': marginal,
        ...
        'likelihood_ratio_measured_c': np.log(np.maximum(measured_h, 1e-12))
                                       - np.log(np.maximum(measured_z, 1e-12)),
